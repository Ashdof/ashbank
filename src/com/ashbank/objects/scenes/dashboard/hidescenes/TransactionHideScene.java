package com.ashbank.objects.scenes.dashboard.hidescenes;
...
public class TransactionHideScene {

    /* ================ DATA MEMBERS ================ */
    private static final Logger logger = Logger.getLogger(TransactionHideScene.class.getName());
    private Scene transactionHideScene;
    ...
    /**
     * Hide Transaction Scene:
     * construct a new scene to hide a transaction object
     * @param sceneController the scene controller
     */
    public TransactionHideScene(SceneController sceneController) {
        this.sceneController = sceneController;
    }

    /**
     * Hide Scene
     * @return the hide scene
     */
    public Scene getTransactionHideScene() {
        return transactionHideScene;
    }
     ...
     */
    public ScrollPane createTransactionHideRoot(String transactionID) throws SQLException {
...
//        ).getFullName();
        ...
        lblInstruction = new Label("Hide Transaction Record");
        ...
        gridPaneButtons = this.createTransactionHideSceneButtons(transactionID);
        vBoxDataPane = this.createTransactionHideDataPane(transactions);
     ...
     */
    private VBox createTransactionHideDataPane(BankAccountTransactions bankAccountTransactions) throws SQLException {
        VBox vBoxRoot;
        HBox hBoxWarning;
        GridPane gridPane;
    ...
    /**
     * Buttons:
     * create the control buttons on the hide scene
     * @param transactionID the ID of the transaction object
     * @return an HBox node containing the buttons
     */
    private GridPane createTransactionHideSceneButtons(String transactionID) {

        GridPane gridPane;
        Button btnCancel, btnHide;
        Separator sep1;
        ...
        btnHide.setOnAction(e -> {
            try {
                ...
                deleteResult = bankTransactionsStorageEngine.hideBankTransactionData(transactionID);
        ...
        gridPane.setPadding(new Insets(10));
        GridPane.setHgrow(btnHide, Priority.NEVER);

        sep1 = new Separator(Orientation.VERTICAL);

        gridPane.add(btnCancel, 0, 0);
        gridPane.add(sep1, 1, 0);
        gridPane.add(btnHide, 2, 0);
